...
func GetSpecs(context *gin.Context) ([]model.Spec, helpers.PaginationData) {
	...
	helpers.DB.Model(&model.Spec{}).Count(&totalItems)
	pagination := helpers.GetPaginationData(page, perPage, totalItems, sort, model.Spec{})
	helpers.DB.Preload(clause.Associations).Clauses(clauses...).Where(queriesMap).Order("created_at " + sort).Limit(perPage).Offset(pagination.Offset).Find(&specs)
	return specs, pagination
}
